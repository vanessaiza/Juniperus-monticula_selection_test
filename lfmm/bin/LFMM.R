...
pdf("sNMF.pdf")
plot(project, col = "royalblue4", pch = 19, cex = 1.2)
dev.off()
...
par(mfrow = c(4,1))
hist(p.pdry.adj, col = "royalblue4", main = "Precipitation of Driest Quarter", xlab='',cex.lab=1.3)
hist(p.pwarm.adj, col = "royalblue3", main = "Precipitation of Warmest Quarter", xlab='',cex.lab=1.3)
hist(p.iso.adj, col = "dodgerblue3", main = "Isothermality ", xlab='',cex.lab=1.3)
hist(p.mtdry.adj, col = "skyblue2", main = "Mean Temperature of Driest Quarter", xlab=expression(italic(P-value)),cex.lab=1.3)
dev.off()
...
q.warm<-qvalue(p.pwarm.adj)$qvalues
...
par(mfrow = c(4,1))
plot(-log10(q.pdry), pch = 19, col = "royalblue4", main = "Precipitation of Driest Quarter", cex = .7, xlab = '', ylim=c(0,12),cex.lab=1.3)
plot(-log10(q.warm), pch = 19, col = "royalblue3", main = "Precipitation of Warmest Quarter", cex = .7, xlab = '',ylim=c(0,12),cex.lab=1.3)
plot(-log10(q.iso), pch = 19, col = "dodgerblue3",main = "Isothermality", cex = .7, xlab = '',ylim=c(0,12),cex.lab=1.3)
plot(-log10(q.mtdry), pch = 19, col = "skyblue2", main = "Mean Temperature of Driest Quarter", cex = .7, xlab = "SNP (ordered by contig arbitrarily)", ylim=c(0,12),cex.lab=1.3)
dev.off()
...
outliers_results<-cbind(sum(q.pdry<0.01 & abs(z.pdry)>2),sum(q.warm<0.01 & abs(z.pwarm)>2),sum(q.iso<0.01 & abs(z.iso)>2),sum(q.mtdry<0.01 & abs(z.mtdry)>2))
outliers_results
...
#We can combine all the z and Q-values into a table and add the column snp_position
lfmm.results <- cbind(z.pdry, q.pdry, z.pwarm, q.warm, z.iso, q.iso, z.mtdry, q.mtdry)
snp_position<-c(1:2904)
lfmm.results1<-cbind(lfmm.results, snp_position)
...
library(dplyr)
#Filter to check the outlier list (snp_position)
